...
open class ApiClient(val baseUrl: String) {
  companion object {
    ...
    protected const val JsonMediaType = "application/vnd.api+json"
  ...
  protected inline fun <reified T : Any?> request(
    ...
    val httpUrl = baseUrl.toHttpUrlOrNull() ?: error("baseUrl is invalid.")
    var urlBuilder =
      httpUrl.newBuilder()
    ...
    if ((headers[ContentType] ?: "") == "") {
      error("Missing Content-Type header. This is required.")
    }

    if ((headers[Accept] ?: "") == "") {
      error("Missing Accept header. This is required.")
    }
    ...
    var request: Request.Builder = // TODO: support multiple contentType,accept options here.
    ...
    return apiInfrastructureResponse<T>(response, accept)
  }

  protected inline fun <reified T : Any?> apiInfrastructureResponse(
    response: Response,
    accept: String,
  ): ApiInfrastructureResponse<T?> {
    when { // TODO: handle specific mapping types. e.g. Map<int, Class<?>>
